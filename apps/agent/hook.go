...
// runHook is `ccxd hook`. It reads the hook payload from stdin, hands it to the
// running ccxd over the local socket, and returns. It does nothing else — no
// retry, no forwarding, no parsing of the payload (#18: the hook stays thin;
// forwarding and retry are ccxd's job).
//
...
func runHook(socketPath, incomingDir string, stdin io.Reader) int {
	...
	_ = writeIncoming(incomingDir, payload)
	return 0
}
