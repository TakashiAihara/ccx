...
const (
	...
	caveat      = `,"isMeta":true,"message":{"role":"user","content":"<local-command-caveat>Caveat: The messages below were generated by the user while running local commands.</local-command-caveat>"}`
...
func TestStepCountsFromTheStartOfTheLastRequest(t *testing.T) {
	...
	// 11:05: past the hour from the request's start, before the hour from the
	// answer (11:12), so only the request-start clock says expired.
...
func TestStepWaitsForARunningTurn(t *testing.T) {
	...
	for _, c := range []struct {
		...
		{"a prompt interrupted by Esc before any answer", base + rec("user", "10:49:00", human) + rec("user", "10:49:30", interrupted), 1},
		{"a /model with its caveat record, as written for real", base + rec("user", "10:49:05", caveat) + rec("user", "10:49:05", cmd) + rec("user", "10:49:05", out), 1},
		{"all tools returned, no answer yet, 5 minutes on", rec("user", "10:00:00", human) + rec("assistant", "10:00:05", toolUse) + rec("user", "10:44:00", toolRes), 1},
...
func TestStepCountsFromTheToolResultsRequest(t *testing.T) {
	...
	for _, c := range []struct {
		now  string
		want int
	}{{"10:59:59", 0}, {"11:00:00", 1}} {
		h, pushes, _ := fixture(t, tr, at(c.now))
		var sent time.Time
		h.step(&sent)
		if *pushes != c.want {
			t.Errorf("at %s: pushes=%d, want %d", c.now, *pushes, c.want)
		}
	}
}
