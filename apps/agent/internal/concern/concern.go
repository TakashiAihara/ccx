// Package concern is the seam that makes ccx-agent a modular monolith (ADR 0002).
//
// ccx-agent is one process, but its three jobs — collect (hooks → center), carry
...
// A Concern is one of ccx-agent's bundled jobs. Run blocks until ctx is cancelled (or
...
// cancelled so the rest wind down too — ccx-agent lives and dies as one process
// (ADR 0002).
func Run(ctx context.Context, log func(string, ...any), concerns ...Concern) error {
	if len(concerns) == 0 {
		log("no concern is enabled — ccx-agent is idle (all concerns off)")
		<-ctx.Done()
		return nil
	}
